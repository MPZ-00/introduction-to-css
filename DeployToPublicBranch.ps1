# Deploy Hugo public/ to a GitHub Pages branch.
# All project-specific values are derived from the repo, nothing to update when copying.

param(
	[string]$Branch = "public"
)

# Guard: must run from the Hugo site root
if (-not (Test-Path "hugo.toml")) {
	Write-Host "Error: hugo.toml not found. Run this script from the repository root." -ForegroundColor Red
	exit 1
}
...
$hasUpdateIndices = Test-Path "$PSScriptRoot\tools\UpdateIndices.ps1"
...
if ($hasUpdateIndices) {
	...
	& "$PSScriptRoot\tools\UpdateIndices.ps1"
